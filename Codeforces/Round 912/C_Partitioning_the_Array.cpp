...
void solve()
{
    ...
    // int num=arr[0];
    // for(int i=1;i<n;i++)num=__gcd(num,arr[i]);
    // if(num!=1){
    //     int cnt=0;
    //     for(int i=1;i*i<=n;i++){
    //         if(n%i==0){
    ...
    for(int i=1;i*i<=n;i++){
        if(n%i==0){
            int prev=0;
            for(int k=0;k<i;k++){
                int num=abs(arr[k]-arr[k+i]);
                for(int j=k+2*i;j<n;j+=i){
                    num=__gcd(num,abs(arr[j]-arr[k]));
                }
                prev=__gcd(prev,num);
            }
            if(prev!=1){ans++;}
            if(i!=n/i && i!=1){
                prev=0;
                for(int k=0;k<n/i;k++){
                    int num=abs(arr[k]-arr[k+n/i]);
                    for(int j=k+2*(n/i);j<n;j+=n/i){
                        num=__gcd(num,abs(arr[j]-arr[k]));
                    }
                    prev=__gcd(prev,num);
                }
